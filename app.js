...
function onPlayerReady(event) {
    // Player pronto, imposta il volume iniziale
    player.setVolume(document.getElementById('volumeSlider').value);
    startProgressLoop();
}

function onPlayerStateChange(event) {
    // Gestione degli stati (0 = terminato, 1 = in riproduzione, 2 = in pausa)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!appState.apiKey) {
        setTimeout(() => { alert("Benvenuto! Per iniziare a cercare, configura la tua YouTube API Key cliccando sull'icona dell'ingranaggio in alto a destra."); }, 1000);
    }
});

// Funzione di ricerca su YouTube API
...
// Mostra i risultati a schermo tipo lista traccia di Spotify
...
// Caricamento e riproduzione traccia
function loadTrack(index) {
    ...
    // Rigenera evidenziazione nella lista traccia
    renderTracklist();

    // Carica il video di YouTube
...
function nextTrack() {
    ...
    if (nextIdx >= appState.tracks.length) nextIdx = 0; // Loop all'inizio
    loadTrack(nextIdx);
}
...
function prevTrack() {
    ...
    if (prevIdx < 0) prevIdx = appState.tracks.length - 1; // Vai all'ultima
    loadTrack(prevIdx);
}

// Avanzamento e Tracciamento del tempo
